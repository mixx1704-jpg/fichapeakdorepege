...
import { SupplementOrganPerks } from "./SupplementPerks";
import { KakujaPanel } from "./KakujaPanel";
import { blankKakujaState, type KakujaState } from "./kakuja-data";
import { kakujaPerksCost, normalizeKakujaPerks } from "./kakuja-perks";
import {
  calculateCustomSkillBonuses,
  customSkillsCost,
  CustomSkillsPanel,
...
type SaveFile = { version: 7; activeId: string; characters: CharacterSheet[] };
const STORAGE_KEY = "fome-justica-fichas-v2";
const THEME_KEY = "fome-justica-theme";

export const gradeProgression = [
  { from: 2, to: 4, requiredPE: 10 },
  { from: 4, to: 6, requiredPE: 20 },
  { from: 6, to: 8, requiredPE: 30 },
  { from: 8, to: 10, requiredPE: 40 },
...
export function cumulativeGradePE(grade: number) {
  return gradeProgression
    .filter((step) => step.to <= grade)
    .reduce((total, step) => total + step.requiredPE, 0);
}

function legacyCumulativeGradePE(grade: number) {
...
export function progressionPEFor(state: { grade: number; earnedPE: number; progressPE: number }) {
  return Math.max(state.earnedPE, cumulativeGradePE(state.grade) + state.progressPE);
}

export function registerEarnedPE<T extends { grade: number; earnedPE: number; progressPE: number }>(state: T, amount: number): T {
  const gained = Math.max(0, Math.floor(amount));
  if (!gained) return state;
  return {
    ...state,
    earnedPE: progressionPEFor(state) + gained,
    progressPE: state.grade < 14 ? state.progressPE + gained : 0,
  };
}

export function advanceGradeIfReady<T extends { grade: number; progressPE: number }>(state: T): T {
  const requiredPE = gradeRequirementFor(state.grade);
  if (!requiredPE || state.progressPE < requiredPE || state.grade >= 14) return state;
  return { ...state, grade: state.grade + 2, progressPE: 0 };
}
...
export function normalizeCharacter(input: Partial<CharacterSheet>, sourceVersion = 2): CharacterSheet {
  ...
  const normalizedGrade = Number(input.grade ?? base.grade) || base.grade;
  const normalizedProgressPE = Math.max(0, Number(input.progressPE) || 0);
  const savedEarnedPE = Math.max(0, Number(input.earnedPE) || 0);
  const legacyGradeGrant = legacyCumulativeGradePE(normalizedGrade);
  const normalizedEarnedPE = sourceVersion === 5 && savedEarnedPE >= legacyGradeGrant
    ? cumulativeGradePE(normalizedGrade) + (savedEarnedPE - legacyGradeGrant)
    : savedEarnedPE;
  return {
...
export function evolutionCostsForSheet(item: Evolution, rank: number, species: SpeciesId) {
  const baseIncrements = Array.from({ length: Math.max(0, rank) }, (_, index) => item.rankCosts?.[index] ?? item.cost);
  const paidIncrements = baseIncrements.map((cost) => species === "ghoul-dominante" ? Math.max(1, cost - 3) : cost);
  return {
    baseTotal: baseIncrements.reduce((total, cost) => total + cost, 0),
    paidTotal: paidIncrements.reduce((total, cost) => total + cost, 0),
    paidIncrements,
  };
}
...
export function calculateRegeneration(maxLife: number, grade: number, normalRank: number, superiorRank: number) {
  ...
  const superiorBase = superiorRank === 1
    ? Math.ceil(maxLife / 6)
    : superiorRank === 2
      ? Math.ceil(maxLife / 4)
      : superiorRank >= 3
        ? Math.ceil(maxLife / 2)
        : 0;
  const superior = superiorRank > 0 ? superiorBase + grade : 0;
...
export function perkCostForSheet(item: Perk, rank: number, sheet: CharacterSheet) {
...
export function perkRequirementMet(perk: Perk, sheet: CharacterSheet, permanentAttributes: Record<AttributeKey, number>) {
  if (perk.requiredWeapon && sheet.weaponKind !== perk.requiredWeapon) return false;
  if (perk.requiredWeapon === "kagune" && !speciesOptions.find(s => s.id === sheet.species)?.hasKagune) return false;
  if (perk.requiresKakuja && (!["ghoul", "ghoul-dominante", "ghoul-artificial"].includes(sheet.species) || sheet.kakuja.cannibalPE < 25)) return false;
  if (perk.minKaguneTypes && sheet.species === "quinx") return false;
  if (perk.requiresRC && !speciesOptions.find(s => s.id === sheet.species)?.hasKagune) return false;
...
export default function Home() {
  ...
  const derived = useMemo(() => {
    ...
    attributeKeys.forEach((key) => { permanentDice[key] += customSkillBonuses.attributeDice[key]; });
    ...
    if (kaguneManifested && kaguneFamilies.has("Koukaku")) kaguneRD += 5 * evolutionRank(sheet.selectedEvolutions, "revestimento-exoesqueletico");
    ...
    let rawRD = characterRD + kaguneRD + customSkillBonuses.rd;
    ...
    let nominalEvolutionPE = 0;
    let evolutionPE = 0;
    sheet.selectedEvolutions.forEach((id) => {
      const item = evolutions.find((candidate) => candidate.id === id);
      if (!item) return;
      const nextRank = (evolutionCounts.get(id) || 0) + 1;
      evolutionCounts.set(id, nextRank);
      const costs = evolutionCostsForSheet(item, nextRank, sheet.species);
      nominalEvolutionPE += costs.baseTotal - evolutionCostsForSheet(item, nextRank - 1, sheet.species).baseTotal;
      evolutionPE += costs.paidTotal - evolutionCostsForSheet(item, nextRank - 1, sheet.species).paidTotal;
    });
    ...
    const completedGradeSteps = gradeProgression.filter((step) => step.to <= sheet.grade);
    const currentGradeRequirement = completedGradeSteps.at(-1)?.requiredPE || 0;
    const cumulativeGradeRequirement = cumulativeGradePE(sheet.grade);
    const hasNextGrade = sheet.grade < 14;
    const nextGrade = hasNextGrade ? sheet.grade + 2 : sheet.grade;
    const nextGradeRequirement = hasNextGrade ? gradeRequirementFor(sheet.grade) : 0;
    ...
    const gradeRewardBreakdown = completedGradeSteps.map((step) => step.requiredPE);
    const progressionPE = progressionPEFor(sheet);
    const progressRemaining = Math.max(0, nextGradeRequirement - sheet.progressPE);
    const progressPercent = hasNextGrade && nextGradeRequirement > 0 ? Math.min(100, (sheet.progressPE / nextGradeRequirement) * 100) : 100;
    const canAdvanceGrade = hasNextGrade && sheet.progressPE >= nextGradeRequirement;
    const peAvailable = 20 + species.creationGrant + progressionPE + sheet.extraPE + drawbackCredit;
    const kakujaCommonPE = Math.max(0, sheet.kakuja.extraKakujaPE || 0) + kakujaPerksCost(sheet.kakuja.selectedPerks);
    const peSpent = species.subCost + quimeraCost + attributePE + perksPE + paidEffectPE + evolutionPE + customSkillsPE + kakujaCommonPE;
    const peRemaining = peAvailable - peSpent;

    const gradeMultiplier = Math.max(1, sheet.grade / 2);
    const maxLife = Math.max(1, Math.floor(10 + activeAttributes.vigor + species.lifeBase * gradeMultiplier + (sheet.perks.resistente ? Math.min(sheet.grade, 10) : 0) + sheet.customBonuses.hp + customSkillBonuses.maxLife));
    let maxSanity = 10 + activeAttributes.controle - (sheet.selectedEvolutions.includes("monstro") ? 5 : 0) + sheet.customBonuses.sanity + customSkillBonuses.maxSanity;
    if (sheet.drawbacks["mente-fragil"]) maxSanity = Math.floor(maxSanity / 2);
    ...
    const customSkillRC = sheet.species === "ghoul-dominante" ? customSkillsPE + sheet.customSkills.length : customSkillsPE;
    const maxRC = sheet.weaponKind === "kagune" ? Math.max(0, effectRC + evolutionRC + kagunePerkRC + customSkillRC + quimeraCost + activeAttributes.vigor + 4 + sheet.customBonuses.rc + customSkillBonuses.maxRC) : 0;
    const rd = Math.max(0, rawRD);
    ...
    const regeneration = passiveRegeneration(sheet.vats, {
  ...
  const filteredPerks = useMemo(() => {
    ...
    return perks.filter((item) => item.category !== "Kagune Quimera" && item.location !== "kakuhou" && (perkCategory === "Todas" || item.category === perkCategory) && (!query || `${item.sourceId || ""} ${item.name} ${item.description} ${item.requirement || ""}`.toLocaleLowerCase("pt-BR").includes(query)));
  ...
  const registerProgressPE = () => {
    const gained = Math.max(0, Math.floor(peGainInput));
    if (!gained) return;
    updateSheet((current) => registerEarnedPE(current, gained));
    setPeGainInput(0);
  };
  const advanceGrade = () => updateSheet(advanceGradeIfReady);
  ...
  const duplicateCharacter = () => { const duplicate = normalizeCharacter({ ...sheet, id: uid(), name: `${sheet.name || "Personagem"} — cópia`, updatedAt: Date.now() }, 7); setCharacters((current) => [...current, duplicate]); setActiveId(duplicate.id); setShowMenu(false); };
  ...
  const exportData = () => {
    const blob = new Blob([JSON.stringify({ version: 7, activeId, characters } satisfies SaveFile, null, 2)], { type: "application/json" });
        ...
        <section className="content">
          ...
          {tab === "resumo" && <div className="page-stack">
            ...
            <div className="vats-entry"><div><span>MONITOR CORPORAL</span><strong>{Object.values(sheet.vats.body).filter(r=>r.damage>0||r.effects.length||r.destroyed).length} / 6 regiões afetadas</strong><small>Vida regional é independente da Vida geral, como no VATS original.</small></div><button type="button" onClick={()=>setTab("vats")}>Abrir VATS ↗</button></div><section className="summary-columns">
              <div className="section-block compact">
                ...
                <div className="ledger"><Ledger label="Verba total" value={derived.peAvailable} /><Ledger label="PE aplicado pela progressão" value={derived.progressionPE} positive /><Ledger label="Atributos" value={derived.attributePE} negative /><Ledger label="Vantagens do catálogo" value={derived.nonKaguneCatalogPerksPE} negative /><Ledger label="Vantagens personalizadas" value={derived.customPerksPE} negative /><Ledger label="Kakuhou / arma" value={derived.paidEffectPE + derived.evolutionPE + derived.quimeraCost + derived.kagunePerksPE + derived.customSkillsPE} negative /><Ledger label="Kakuja · PE comum" value={derived.kakujaCommonPE} negative /><Ledger label="Subespécie" value={derived.species.subCost} negative /><Ledger label="Desvantagens" value={derived.drawbackCredit} positive /></div>
                <div className="inline-controls single"><label><span>PE adicionais da mesa</span><input type="number" value={sheet.extraPE} onChange={(event) => patchSheet({ extraPE: Number(event.target.value) || 0 })} /></label></div>
                <div className="grade-progress">
                  <div className="grade-progress-head"><span>Progressão de Grau</span><strong>Grau {sheet.grade}{derived.hasNextGrade ? ` → ${derived.nextGrade}` : " · máximo"}</strong></div>
                  {derived.hasNextGrade ? <>
                    <div className="grade-progress-values"><b>{sheet.progressPE} / {derived.nextGradeRequirement} PE</b><small>{derived.canAdvanceGrade ? "Requisito cumprido" : `${derived.progressRemaining} PE restantes`}</small></div>
                    <div className="grade-progress-meter" role="progressbar" aria-label={`Progresso para o Grau ${derived.nextGrade}`} aria-valuemin={0} aria-valuemax={derived.nextGradeRequirement} aria-valuenow={Math.min(sheet.progressPE, derived.nextGradeRequirement)}><i style={{ width: `${derived.progressPercent}%` }} /></div>
                  </> : <p className="grade-max-message">O Grau 14 é o máximo. Novos PE ainda entram normalmente na verba da ficha.</p>}
                  <div className="pe-gain-row"><label><span>PE ganho</span><input type="number" min={0} step={1} value={peGainInput || ""} onChange={(event) => setPeGainInput(Math.max(0, Number(event.target.value) || 0))} onKeyDown={(event) => { if (event.key === "Enter") registerProgressPE(); }} placeholder="Ex.: 5" /></label><button type="button" onClick={registerProgressPE} disabled={peGainInput <= 0}>Registrar PE</button></div>
                  <small className="grade-progress-note">O valor registrado entra imediatamente na verba e no progresso. Ao subir, a barra zera, o PE já ganho permanece e o Grau garante automaticamente o mínimo acumulado da progressão.</small>
                  {derived.hasNextGrade && <button type="button" className={`grade-up-button ${derived.canAdvanceGrade ? "ready" : ""}`} disabled={!derived.canAdvanceGrade} onClick={advanceGrade}>{derived.canAdvanceGrade ? `Subir agora para Grau ${derived.nextGrade}` : `Faltam ${derived.progressRemaining} PE para o Grau ${derived.nextGrade}`}</button>}
                  <div className="grade-requirements" aria-label="Requisitos de progressão por Grau">{gradeProgression.map((step) => <span key={step.from} className={step.to <= sheet.grade ? "completed" : step.from === sheet.grade ? "current" : ""}><b>{step.from} → {step.to}</b><small>{step.requiredPE} PE</small></span>)}</div>
                </div>
              </div>
          ...
          {tab === "kakuhou" && <div className="page-stack"><PageHeader number="04" title="Kakuhou & arma RC" description="Catálogo oficial, combate e habilidades autorais com efeitos ligados aos cálculos da ficha." />
            ...
            {sheet.weaponKind !== "nenhum" && <><section id="kakuhou-efeitos"><div className="catalog-toolbar"><div><span className="section-kicker">Efeitos de criação</span><h2 className="toolbar-title">Catálogo de efeitos</h2></div><label className="search-field"><span>⌕</span><input value={effectSearch} onChange={(event) => setEffectSearch(event.target.value)} placeholder="Buscar efeito" /></label></div><div className="filter-row">{["Compatíveis", "Todos", "Geral", "Ukaku", "Koukaku", "Rinkaku", "Bikaku"].map((family) => <button type="button" key={family} className={effectFamily === family ? "active" : ""} onClick={() => setEffectFamily(family)}>{family}</button>)}</div><div className="catalog-summary"><span>{filteredEffects.length} efeitos</span><strong>{derived.actualEffectPE} PE em efeitos</strong></div><div className="catalog-grid effects">{filteredEffects.map((item) => {
              ...
              {sheet.weaponKind === "kagune" && <section id="kakuhou-quimera"><div className="attribute-section-head"><div><span>Vantagens de Kagune Quimera</span><p>Integram a Kakuhou e entram automaticamente em PE, RC, Dureza e requisitos.</p></div><strong>{derived.kagunePerksPE} PE</strong></div><div className="catalog-summary"><span>{filteredQuimeraPerks.length} opções quiméricas</span><strong>{Object.keys(sheet.perks).filter((id) => perks.find((candidate) => candidate.id === id)?.category === "Kagune Quimera").length} adquiridas</strong></div><div className="catalog-grid effects">{filteredQuimeraPerks.map((item) => {
                const acquired = sheet.perks[item.id]; const eligible = perkRequirementMet(item, sheet, derived.permanentAttributes); const price = perkCostForSheet(item, acquired?.rank || 1, sheet);
            ...
            {kakuhouView === "autorais" && <CustomSkillsPanel skills={sheet.customSkills} onChange={(customSkills) => patchSheet({ customSkills })} />}
          </div>}
...
function DamageCard({ title, kicker, formula, hitFormula, attribute, steps, modifier, controls, counters, conditions, scope, active, onToggle, onCopy, onHitCopy, copied, hitCopied }: {
  ...
  hitFormula: string; onCopy: () => void; onHitCopy: () => void; copied: boolean; hitCopied: boolean;
}) { return <article className="damage-card"><div className="damage-card-head"><div><span>{kicker}</span><h3>{title}</h3></div><div className="damage-metrics"><span>{attribute}</span><b>{steps} Passos</b><b>{modifier >= 0 ? `+${modifier}` : modifier} Mod.</b></div></div><div className="damage-formulas"><button type="button" className="hit-formula" onClick={onHitCopy}><span>{hitCopied ? "Copiado" : "Teste de acerto"}</span><code>{hitFormula}</code><i>⧉</i></button><button type="button" className="damage-formula" onClick={onCopy}><span>{copied ? "Copiado" : "Dano final"}</span><code>{formula}</code><i>⧉</i></button></div><div className="damage-controls">{controls}</div>{counters}<div className="damage-condition-list">{conditions.length ? conditions.map((condition) => { const checked = Boolean(active[`${scope}:${condition.id}`]); return <label key={condition.id} className={checked ? "active" : ""}><input type="checkbox" checked={checked} onChange={() => onToggle(scope, condition.id)} /><span><b>{condition.label}</b><small>{condition.detail}</small></span></label>; }) : <p>Nenhuma condição comprada para este ataque.</p>}</div></article>; }
...
function AttributeCard({ attributeKey, sheet, derived, copied, onCopy, onSet, onExtra }: {
  ...
  const base = sheet.baseAttributes[attributeKey]; const total = derived.activeAttributes[attributeKey]; const speciesBonus = derived.speciesBonuses[attributeKey]; const customBonus = sheet.customBonuses.attributes[attributeKey]; const skillAttributeBonus = derived.customSkillBonuses.attributeBonus[attributeKey]; const skillDice = derived.customSkillBonuses.attributeDice[attributeKey];
